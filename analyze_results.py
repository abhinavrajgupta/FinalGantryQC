...
def get_module_paths(module_name):
    ...
    images_dir_arrowplot = os.path.join(base_dir, module_name, 'ResultsArrowPlots')
    labels_dir_arrowplot = os.path.join(base_dir, module_name, 'ResultsArrowPlots','labels')
    images_dir_wirebond = os.path.join(base_dir, module_name, 'ResultsArrowPlots')
    labels_dir_wirebond = os.path.join(base_dir, module_name, 'ResultsArrowPlots','labels')
    return images_dir_arrowplot,labels_dir_arrowplot,images_dir_wirebond,labels_dir_wirebond

def process_labels(label_path_arrowplot, img_path_arrowplot):
    dict1 = {}
    for name in os.listdir(img_path_arrowplot):
        if ".jpg" in name:
            try:
                image_path = os.path.join(img_path_arrowplot, name)
                mask = cv2.imread(image_path)
                (h, w) = mask.shape[:2]
                txtname = name.split('.jpg')[0] + ".txt"
                label_file_path = os.path.join(label_path_arrowplot, txtname)
...
def detect_hough_circles(img_path_arrowplot):
    dict2 = {}  
    for name in os.listdir(img_path_arrowplot):
        if ".jpg" in name:
            image_path = os.path.join(img_path_arrowplot, name)
...
def process_and_generate_df(img_path_arrowplot, label_path_arrowplot):
    dict1 = process_labels(label_path_arrowplot, img_path_arrowplot)  
    dict2 = detect_hough_circles(img_path_arrowplot)
...
def process_labels_wirebond(label_path_wirebond):
    category_data = {
        'Glue': [],
        'Broken': [],
        'Anomaly': [],
    ...
    for file_name in os.listdir(label_path_wirebond):
        if not file_name.endswith('.txt'):
            continue
        image_name = file_name.replace(".txt", ".jpg")
        file_path = os.path.join(label_path_wirebond, file_name)
...
def create_histogram(category_data, images_path_wirebond):
    ...
    save_path = os.path.join(images_path_wirebond, 'Histogram.jpg')
    plt.savefig(save_path, dpi=100)
    plt.close()
    print(f"Histogram saved at {save_path}")

def create_summary_table(category_data, images_path_wirebond):
    data_summary = {
    ...
    summary_file_path = os.path.join(images_path_wirebond, 'Summary_Table.csv')
    summary_df.to_csv(summary_file_path, index=False)
    print(f"Summary table saved at: {summary_file_path}")

def main():
    module_name = input("Enter the module name (e.g., 'M35'): ")
    images_path_arrowplot, labels_path_arrowplot, images_path_wirebond, labels_path_wirebond = get_module_paths(module_name)
    df1, df3 = process_and_generate_df(images_path_arrowplot, labels_path_arrowplot)
    df3_sorted = process_files_and_append_to_df(images_path_arrowplot,df3)
    final_df = finaldf(df1, df3_sorted)
    create_arrow_plot(final_df, df3, images_path_arrowplot)
    category_data = process_labels_wirebond(labels_path_wirebond)
    create_histogram(category_data, images_path_wirebond)
    create_summary_table(category_data, images_path_wirebond)
